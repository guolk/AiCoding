...
const EXAM_API_BASE = 'http://localhost:8000/api';

const getExamToken = (): string | null => {
...
const examApiCall = async <T>(
  ...
  if (method !== 'GET' && data) {
    config.body = JSON.stringify(convertKeysToSnake(data));
  }
  ...
  return convertKeysToCamel(responseData) as T;
};

export const examApi = {
  async login(username: string, password: string) {
    const result = await examApiCall<{ accessToken: string; user: any; tokenType: string }>(
      `${EXAM_API_BASE}/auth/login`, 'POST', { username, password }
    );
    sessionStorage.setItem('exam_token', result.accessToken);
    return result;
  },

  async register(username: string, email: string, password: string, role: string = 'student') {
    return examApiCall(`${EXAM_API_BASE}/auth/register`, 'POST', { 
  ...
  async getQuestions(params: {
    ...
    return examApiCall(`${EXAM_API_BASE}/questions`, 'GET', params);
  },

  async getQuestion(id: number) {
    return examApiCall(`${EXAM_API_BASE}/questions/${id}`, 'GET');
  },

  async createQuestion(data: any) {
    return examApiCall(`${EXAM_API_BASE}/questions`, 'POST', data);
  },

  async updateQuestion(id: number, data: any) {
    return examApiCall(`${EXAM_API_BASE}/questions/${id}`, 'PUT', data);
  },

  async deleteQuestion(id: number) {
    return examApiCall(`${EXAM_API_BASE}/questions/${id}`, 'DELETE');
  },

  async getPapers(params: { page?: number; pageSize?: number; search?: string } = {}) {
    return examApiCall(`${EXAM_API_BASE}/papers`, 'GET', params);
  },

  async getPaper(id: number) {
    return examApiCall(`${EXAM_API_BASE}/papers/${id}`, 'GET');
  },

  async createPaper(data: any) {
    return examApiCall(`${EXAM_API_BASE}/papers`, 'POST', data);
  },

  async deletePaper(id: number) {
    return examApiCall(`${EXAM_API_BASE}/papers/${id}`, 'DELETE');
  },

  async getExams(params: { page?: number; pageSize?: number; status?: string } = {}) {
    return examApiCall(`${EXAM_API_BASE}/exams`, 'GET', params);
  },

  async getMyExams() {
    return examApiCall(`${EXAM_API_BASE}/exams/my`, 'GET');
  },

  async getExam(id: number) {
    return examApiCall(`${EXAM_API_BASE}/exams/${id}`, 'GET');
  },

  async createExam(data: any) {
    return examApiCall(`${EXAM_API_BASE}/exams`, 'POST', data);
  },

  async updateExam(id: number, data: any) {
    return examApiCall(`${EXAM_API_BASE}/exams/${id}`, 'PUT', data);
  },

  async startExam(examId: number) {
    return examApiCall(`${EXAM_API_BASE}/exams/${examId}/start`, 'POST');
  },

  async saveDraft(examId: number, answers: any[]) {
    return examApiCall(`${EXAM_API_BASE}/exams/${examId}/save-draft`, 'POST', { answers });
  },

  async submitExam(examId: number, answers?: any[]) {
    const body = answers ? { answers } : {};
    return examApiCall(`${EXAM_API_BASE}/exams/${examId}/submit`, 'POST', body);
  },

  async getSubmission(id: number) {
    return examApiCall(`${EXAM_API_BASE}/submissions/${id}`, 'GET');
  },

  async getExamSubmissions(examId: number) {
    return examApiCall(`${EXAM_API_BASE}/exams/${examId}/submissions`, 'GET');
  },

  async manualGrade(data: { submissionId: number; questionId: number; score: number; comment?: string }) {
    return examApiCall(`${EXAM_API_BASE}/grading/manual`, 'POST', data);
  },

  async finalizeGrading(submissionId: number) {
    return examApiCall(`${EXAM_API_BASE}/grading/finalize/${submissionId}`, 'POST');
  },

  async getExamAnalytics(examId: number) {
    return examApiCall(`${EXAM_API_BASE}/exams/${examId}/analytics`, 'GET');
  },

  logEvent(examId: number, eventType: string, details?: any) {
    console.log(`[AntiCheat] Exam ${examId}: ${eventType}`, details);
  },
};
