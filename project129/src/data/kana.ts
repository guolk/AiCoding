import { KanaProgress, KanaType } from '@/types';
...
export function createInitialKanaProgress(type: KanaType): KanaProgress[] {
  const rows = type === 'hiragana' ? hiraganaRows : katakanaRows;
  const result: KanaProgress[] = [];
  rows.forEach(row => {
    row.forEach(kana => {
      if (kana.character) {
        result.push({
