...
export class PolylineConnection extends BaseConnection {
  ...
  containsPoint(point: Point, scale: number): boolean {
    ...
    for (let i = 0; i < this.points.length - 1; i++) {
      ...
      if (length === 0) {
        if (distance(point, p1) <= tolerance) return true;
        continue;
      }
  ...
  private calculatePathWithObstacles(
    start: Point,
    end: Point,
    ...
    while (true) {
      const directPathClear = expandedObstacles.every(
        (o) => !lineIntersectsRect(currentPoint, end, o)
      );
