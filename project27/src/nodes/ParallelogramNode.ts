...
export class ParallelogramNode extends BaseNode {
  private skew: number = 0.2;

  constructor(config: Partial<INode> & { skew?: number }) {
    super({
      ...config,
  ...
  containsPoint(point: Point): boolean {
    ...
    return (
      this.pointInTriangle(point, triangle1) ||
      this.pointInTriangle(point, triangle2)
    );
  }

  private pointInTriangle(point: Point, triangle: Point[]): boolean {
    const [p0, p1, p2] = triangle;

    const s =
      p0.x * (p2.y - p0.y) - p0.y * (p2.x - p0.x) + point.x * (p0.y - p2.y) + point.y * (p2.x - p0.x);
    const t =
      p0.x * (p1.y - p0.y) - p0.y * (p1.x - p0.x) + point.x * (p0.y - p1.y) + point.y * (p1.x - p0.x);
