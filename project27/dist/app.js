import { FlowchartEngine, NodeType, AlignType, DistributeType } from './index.js';
let engine = null;
function getCanvas() {
    return document.getElementById('flowchart-canvas');
}
...
function initEngine() {
    ...
    engine.setViewState({ x: centerX - 200, y: centerY - 200 }, 1);
    createSampleNodes();
    updateButtons();
    updateStatus();
}
...
function getCenterWorldPosition() {
    const canvas = getCanvas();
    const centerX = canvas.width / 2;
    const centerY = canvas.height / 2;
    const viewState = engine.getViewState();
...
function addNodeWithOffset(type, title, color) {
    if (!engine)
        return;
...
function setupEventListeners() {
    ...
    document.getElementById('export-json')?.addEventListener('click', () => {
        ...
        const contentEl = document.getElementById('json-export-content');
        if (contentEl)
            contentEl.value = json;
    ...
    document.getElementById('import-json')?.addEventListener('click', () => {
        const contentEl = document.getElementById('json-import-content');
        if (contentEl)
            contentEl.value = '';
