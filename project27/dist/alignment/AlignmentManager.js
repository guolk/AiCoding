import { AlignType, DistributeType } from '../types/index.js';
export class AlignmentManager {
    align(nodes, alignType) {
        if (nodes.length < 2)
            return new Map();
    ...
    distribute(nodes, distributeType) {
        if (nodes.length < 3)
            return new Map();
        const positions = new Map();
        const bounds = this.getNodesBounds(nodes);
    ...
    sortNodes(nodes, bounds, distributeType) {
    ...
    calculateTotalNodeWidth(nodes, bounds) {
        let total = 0;
        for (const node of nodes) {
            const bound = bounds.get(node.id);
            if (bound) {
    ...
    calculateTotalNodeHeight(nodes, bounds) {
        let total = 0;
        for (const node of nodes) {
            const bound = bounds.get(node.id);
            if (bound) {
