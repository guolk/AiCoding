...
export function TableCell({ children, selectable = false, row, className = '', title }: TableCellProps) {
  const { selectedRows, toggleRow, getRowId } = useTableContext();
  const isSelected = row ? selectedRows.includes(getRowId(row)) : false;

  if (selectable && row) {
    return (
      <td className={`px-6 py-4 whitespace-nowrap w-12 ${className}`} title={title}>
        <input
          type="checkbox"
          checked={isSelected}
          onChange={(e) => {
  ...
  return (
    <td className={`px-6 py-4 whitespace-nowrap text-sm text-gray-900 ${className}`} title={title}>
      {children}
    </td>
  );
}
