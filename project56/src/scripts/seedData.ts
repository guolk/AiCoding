...
const seedData = async () => {
  ...
  for (const book of books) {
    await new Promise<void>((resolve, reject) => {
      db.run(
        ...
        [book.id, book.isbn, book.title, book.author, book.publisher, book.publishedDate, book.description, book.coverUrl, book.pageCount, book.status, book.readCount, book.currentPage],
        (err) => {
          if (err) reject(err);
          else resolve();
        }
  ...
  const tags = [
    { id: uuidv4(), name: '经典文学', color: '#EF4444' },
    { id: uuidv4(), name: '科幻', color: '#3B82F6' },
    { id: uuidv4(), name: '历史', color: '#10B981' },
    { id: uuidv4(), name: '哲学', color: '#F59E0B' },
    { id: uuidv4(), name: '外国文学', color: '#8B5CF6' },
    { id: uuidv4(), name: '中国文学', color: '#EC4899' }
  ];
  ...
  for (const [bookId, tagId] of bookTagRelations) {
    await new Promise<void>((resolve, reject) => {
      db.run(
        `INSERT INTO book_tags (bookId, tagId) VALUES (?, ?)`,
        [bookId, tagId],
        (err) => {
          if (err) reject(err);
          else resolve();
  ...
  for (const session of sessions) {
    await new Promise<void>((resolve, reject) => {
      db.run(
        `INSERT INTO reading_sessions (id, bookId, startTime, endTime, startPage, endPage, pagesRead, duration) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [session.id, session.bookId, session.startTime, session.endTime, session.startPage, session.endPage, session.pagesRead, session.duration],
        (err) => {
          if (err) reject(err);
          else resolve();
        }
  ...
  for (const note of notes) {
    await new Promise<void>((resolve, reject) => {
      db.run(
        `INSERT INTO notes (id, bookId, type, content, pageNumber, chapter) VALUES (?, ?, ?, ?, ?, ?)`,
        [note.id, note.bookId, note.type, note.content, note.pageNumber, note.chapter],
        (err) => {
          if (err) reject(err);
          else resolve();
        }
  ...
  for (const [noteId, topic] of noteTopics) {
    await new Promise<void>((resolve, reject) => {
      db.run(
        `INSERT INTO note_topics (noteId, topic) VALUES (?, ?)`,
        [noteId, topic],
        (err) => {
          if (err) reject(err);
          else resolve();
  ...
  for (const list of readingLists) {
    await new Promise<void>((resolve, reject) => {
      db.run(
        `INSERT INTO reading_lists (id, name, description, isPublic, shareToken) VALUES (?, ?, ?, ?, ?)`,
        [list.id, list.name, list.description, list.isPublic ? 1 : 0, list.shareToken || null],
        (err) => {
          if (err) reject(err);
          else resolve();
        }
  ...
  for (const [listId, bookId] of listBookRelations) {
    await new Promise<void>((resolve, reject) => {
      db.run(
        `INSERT INTO reading_list_books (listId, bookId) VALUES (?, ?)`,
        [listId, bookId],
        (err) => {
          if (err) reject(err);
          else resolve();
  ...
  for (const history of readHistories) {
    await new Promise<void>((resolve, reject) => {
      db.run(
        ...
        [history.id, history.bookId, history.completedDate, history.rating, history.review],
        (err) => {
          if (err) reject(err);
          else resolve();
        }
