...
const readingService = __importStar(require("./services/readingService"));
...
app.post('/api/notes', async (req, res) => {
    try {
        const note = await noteService.createNote(req.body.bookId, req.body.type, req.body.content, req.body.pageNumber, req.body.chapter, req.body.topics);
...
app.put('/api/notes/:id', async (req, res) => {
    try {
        const note = await noteService.updateNote(req.params.id, req.body.content, req.body.pageNumber, req.body.chapter, req.body.topics);
        if (!note) {
            res.status(404).json({ error: 'Note not found' });
        }
        else {
...
app.post('/api/reading-lists', async (req, res) => {
    try {
        const list = await listService.createReadingList(req.body.name, req.body.description, req.body.isPublic);
...
app.put('/api/reading-lists/:id', async (req, res) => {
    try {
        const list = await listService.updateReadingList(req.params.id, req.body.name, req.body.description, req.body.isPublic);
        if (!list) {
            res.status(404).json({ error: 'Reading list not found' });
        }
        else {
...
app.get('/api/recommendations', async (req, res) => {
    try {
        const recommendations = await listService.getRecommendations(parseInt(req.query.limit) || 10);
...
app.post('/api/books/:id/complete', async (req, res) => {
    try {
        await listService.addCompletedBook(req.params.id, req.body.rating, req.body.review);
        const book = await bookService.getBook(req.params.id);
        res.json(book);
    }
    catch (error) {
